...
private:
    ...
    bool InBoard(const cv::Point2f &pt){
    ...
    void RejectWithF(){
        if (_forw_pts.size() >= 8){
            std::vector<cv::Point2f> un_cur_pts(_cur_pts.size()), un_forw_pts(_forw_pts.size());
...
//                LOG(INFO) << "tem_pt: " << tem_pt;
    ...
    void UndistortedPoints(){
        ...
        for(int i = 0; i < _cur_pts.size(); ++i){
            ...
            _cur_un_pts.emplace_back(cv::Point2f(un_cur_pt.x(), un_cur_pt.y()));
            _cur_un_pts_map.insert(std::make_pair(_ids[i], cv::Point2f(un_cur_pt.x(), un_cur_pt.y())));
        }
    }
    ...
    void ComputePointsVelocity(){
        if (!_prev_un_pts_map.empty()){
            ...
            for(int i = 0; i < _cur_un_pts.size(); ++i){
                if (_ids[i] != -1){
                    std::map<int, cv::Point2f>::iterator it = _prev_un_pts_map.find(_ids[i]);
                    if (it != _prev_un_pts_map.end()){
                        double v_x = (_cur_un_pts[i].x - it->second.x) / dt;
                        double v_y = (_cur_un_pts[i].y - it->second.y) / dt;
                        _pts_velocity.emplace_back(cv::Point2f(v_x, v_y));
                    }
                    else
                        _pts_velocity.emplace_back(cv::Point2f(0, 0));
                }
                else
                    _pts_velocity.emplace_back(cv::Point2f(0, 0));
            }
...
public:
    ...
    std::vector<cv::Point2f> _cur_un_pts, _prev_un_pts;
    std::vector<cv::Point2f> _pts_velocity;
    // _ids首先赋值为-1,然后在system中判断是否为-1赋值为id.
    std::vector<int> _ids, _track_cnt;
    PinholeCamera _pinhole_camera;
    std::map<int, cv::Point2f> _cur_un_pts_map, _prev_un_pts_map;
