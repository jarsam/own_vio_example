...
void EstimatorOptimization::SetParameter()
{
...
void EstimatorOptimization::ProcessIMU(double dt, const Eigen::Vector3d &linear_acceleration,
...
void EstimatorOptimization::ProcessImage(const std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
...
bool EstimatorOptimization::InitialStructure()
{
    // 检测Imu的可观性
    {
...
bool EstimatorOptimization::RelativePose(Eigen::Matrix3d &relative_R, Eigen::Vector3d &relative_T, int &l)
{
...
bool EstimatorOptimization::VisualInitialAlign()
{
    Eigen::VectorXd x;
...
void EstimatorOptimization::SlideWindow()
{
...
void EstimatorOptimization::SlideWindowOld()
{
    _sum_of_back++;
...
void EstimatorOptimization::SlideWindowNew()
{
    _sum_of_front++;
    _feature_manager.RemoveFront(_frame_count);
}

void EstimatorOptimization::SolveOdometry()
{
    if (_frame_count < svar.GetInt("window_size", 20))
        return;
...
void EstimatorOptimization::BackendOptimizationCeres()
{
...
void EstimatorOptimization::Vector2Double()
{
    for(int i = 0; i <= svar.GetInt("window_size", 20); ++i){
...
void EstimatorOptimization::Double2Vector()
{
...
bool EstimatorOptimization::FailureDetection()
{
...
void EstimatorOptimization::ProblemSolve()
{
...
void EstimatorOptimization::MarginOldFrame()
{
    LossFunction *loss_function = new CauthyLoss(1.0);
    Problem problem(Problem::SLAM_PROBLEM);
    std::vector<std::shared_ptr<VertexPose> > vertex_cam_vec;
...
void EstimatorOptimization::MarginNewFrame()
{
    Problem problem(Problem::SLAM_PROBLEM);
    std::vector<std::shared_ptr<VertexPose> > vertex_cam_vec;
    std::vector<std::shared_ptr<VertexSpeedBias> > vertex_speedbias_vec;
    ...
    // 这个函数中没有添加landmark的vertex, 但是在Marginlize函数中添加了
    problem.Marginalize(marg_vertex, pose_dim);
    _H_prior = problem.GetHessianPrior();
    _b_prior = problem.GetbPrior();
    _err_prior = problem.GetErrPrior();
...
void EstimatorOptimization::BackendOptimizationEigen()
{
